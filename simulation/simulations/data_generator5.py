...
def safe_one_hot(final_unique_id, num_classes):
    ...
    sanitized_ids = torch.where(
        valid_mask, final_unique_id, torch.tensor(0, device=final_unique_id.device)
    )

    # One-hot encode
    one_hot_encoded = F.one_hot(
        sanitized_ids.to(torch.int64), num_classes=num_classes
...
class DataGenerator:
    ...
    def __init__(
        ...
        self.p_sensor = params.data_generation.p_sensor
        self.p_target = params.data_generation.p_target
        self.training_dim = (
            params.tracking.cartesian_dim
            + params.tracking.cartesian_dim // 2
            + params.tracking.cartesian_dim % 2
            + 1
        )
        ...
        self.tracks = params.tracking.tracks
    ...
    def _step(
        ...
        for time in range(total_duration):
            ...
            for t in range(total_targets):
                # Did not emit radar signal
                if not self._bool_select_p(self.p_target) or not final_life[time, t]:
                    continue
                ...
                for s in range(total_sensors):
                    ...
                    if (
                        not self._bool_select_p(self.p_sensor)
                        or not final_life[time, t]
                    ):
                        continue
    ...
    def simulate_life(
        self, n, steps, birth_rate, death_rate, revival_cooldown=100, initial_alive=None
    ):
        """
        ...
        if initial_alive is None:
            state = np.random.choice(
                [1, 0], size=n, p=[birth_rate, 1 - birth_rate]
            )  # 1=alive, 0=dead
        else:
        ...
        for t in range(steps):
            ...
            state = np.where(
                (state == 0) & (birth_prob <= birth_rate) & can_revive, 1, state
            )
