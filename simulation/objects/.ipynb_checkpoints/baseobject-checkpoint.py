...
class BaseObject:
    ...
    def update_sequential_movement(
        self, lst, auto_generate: bool = False
    ):
        assert all(
    ...
    def generate_timestamps(self, truncation = None):
        ...
        for duration, movement in self.sequential:        
            ...
            if result.size > 0:
                final_idx = curr_idx + min(int(duration / self.interval), self.timestamp_coordinates[curr_idx:, :].shape[0])
                try:
                    ...
                    self.timestamp_coordinates[curr_idx:final_idx, :] = result[:final_idx - curr_idx,:]
                    self.location = np.add(
                        self.location, movement.additive_vector((final_idx - curr_idx) * self.interval)
                    )
                    break
